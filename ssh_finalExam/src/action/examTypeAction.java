...
public class examTypeAction extends ActionSupport {
    ...
    public String getAllInfo() {
        ...
        List<ExamType> list = examTypeService.getAllInfo(p, examType);
        ...
        jb.put("code", 0);
        jb.put("msg", "");
        jb.put("count", examTypeService.countExamType(p, examType));
        jb.put("data", jr);
        return SUCCESS;
    }
    ...
    public String saveExamType() {
        ...
        int flag = examTypeService.saveExamType(et);
        if (flag > 0) {
    ...
    public String delExamType() {
        jb = new JSONObject();
        int flag[] = examTypeService.deleteExamType(delId);
        if (flag[0] > 0) jb.put("complete", true);
        else {
            if (flag[1] == 0) {
                jb.put("complete", "服务器错误，请联系管理员");
            } else {
                String type_examName = examTypeService.findExamNameById(flag[1]);
