...
public class AdminOperation extends ActionSupport {
    ...
    @Resource
    private IAdminOpService adminOpService;
    ...
    public String viewAllNotice() {
        ...
        List<Notice> list = adminOpService.viewAllNotice(p, no, s_noticeId, s_pbBeginTime, s_pbEndTime);
        ...
        jb = new JSONObject();
        jb.put("data", array);
        jb.put("count", adminOpService.countNotice(no, s_noticeId, s_pbBeginTime, s_pbEndTime));
        jb.put("code", 0);
        jb.put("msg", "");
        return SUCCESS;
    }
    ...
    public String delNoticeById() {
        jb = new JSONObject();
        int flag = adminOpService.deleteNoticeById(del_noticeId);
        if (flag > 0) {
            jb.put("success", true);
        } else {
    ...
    public String saveNotce() {
        ...
        int flag = adminOpService.saveNotice(notice);
        if (flag > 0) {
            jb.put("success", true);
        } else {
            jb.put("success", false);
    ...
    public String allSignUp() {
        ...
        List<Object[]> list = adminOpService.allSignUp(new Page(page, limit), exam, s_stuName, s_sId, isPrint);
        ...
        jb.put("code", 0);
        jb.put("msg", "");
        jb.put("count", adminOpService.countSignUp(new Page(page, limit), exam, s_stuName, s_sId, isPrint));
        return SUCCESS;
    }

    public String approSignUp() {
        jb = new JSONObject();
        int flag = adminOpService.updateSignUp(signUpId, approVersion);
        if (flag > 0) {
            if (flag == 1) {
    ...
    public String saveCerti() {
        adminOpService.saveCerti(saveSId, certificateId);
        return SUCCESS;
    }
}
