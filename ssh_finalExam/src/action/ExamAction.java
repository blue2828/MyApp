...
public class ExamAction extends ActionSupport {
    ...
    public String getAllInfo() {
        ...
        List<Exam> list = examService.findAllInfo(p, e);
        ...
        jb.put("data", a);
        jb.put("count", examService.countInfo(p, e));
        jb.put("code", 0);
        jb.put("msg", "");
        return SUCCESS;
    }
    ...
    public String getAllType() {
        jb = new JSONObject();
        List<String> list = examService.getAllType();
    ...
    public String getThisType() {
        jb = new JSONObject();
        String examType = (String) examService.getThisType(getTypeId).get(0);
    ...
    public String saveExam() {
        jb = new JSONObject();
        int isHavedName = examService.findExamByName(exam.getExamName());
        if (isHavedName == 1) {
            jb.put("success", false);
            jb.put("errMsg", "考试名已存在");
            return SUCCESS;
        }
        int flag = examService.saveExam(exam, examTypeName);
        if (flag > 0) {
            jb.put("success", true);
        } else {
            jb.put("success", false);
    ...
    public String delExam() {
        jb = new JSONObject();
        int[] flag = examService.deleteExam(delId);
        if (flag[0] > 0) {
            ...
            if (flag[1] == 0) {
                ...
                jb.put("complete", false);
                jb.put("errMsg", "考试-" + examService.findExamNameById(flag[1]) + "下有考生选取该考试，不得删除");
            }
        }
        return SUCCESS;
    }
