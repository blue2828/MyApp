...
public class memberAction extends ActionSupport {
        ...
        if (code.isEmpty()) {
            ...
            if (!code.equalsIgnoreCase((String) session.get("imageCode"))) {
                ...
                if (!StringUtil.isNotEmpty(version)) {
                    List<?> list = memberService.validateMember(member);
                    if (list.size() == 0) {
                        jb.put("success", false);
                    ...
                    int succFlag = memberService.save_memRegister(member);
                    if (succFlag > 0) {
                        jb.put("success", true);
    ...
    public String getMemberImage() throws Exception {
        ...
        if (currentAdmin != null) {
            imgPath = memberService.refreshImgHeader(currentAdmin);
        } else if (currentStu != null) {
            imgPath = memberService.refreshImgHeader(currentStu);
        }
        ...
        if (file == null) {
            ...
            try {
                ...
                if (currentStu != null) {
                    ...
                    succFlag = memberService.updateImg(currentStu);

                } else {
                    currentAdmin.setAdminImg(fileSaveName);
                    succFlag = memberService.updateImg(currentAdmin);
                }
                if (succFlag > 0) {
                    jb.put("success", true);
                    if (imgHeader != null) {
    ...
    public String viewAllReg() {
        ...
        List<Member> list = memberService.viewAllReg(p, s_memName, s_memberId);
        jb = new JSONObject();
        ...
        if (list.size() > 0) {
            ...
            jb.put("count", memberService.countReg(s_memName, s_memberId));
            jb.put("data", array);
        } else {
            jb.put("count", 0);
            jb.put("data", "");
    ...
    public String approReg() {
        ...
        if (StringUtil.isNotEmpty(memberIds)) {
            flag = memberService.updateRegAppro(memberIds);
        } else {
            flag = memberService.updateRegAppro(memberId);
        }
        jb = new JSONObject();
        if (flag > 0) {
            jb.put("success", true);
    ...
    public String getRegCount() {
        int counts = memberService.countReg(s_memName, s_memberId);
        jb = new JSONObject();
    ...
    public String loginRead() {
        ...
        if (loginRead != null) {
            ...
            int flag = memberService.updateLoginRead(stu);
            if (flag > 0) {
    ...
    public String updateMemberInfo() {
        ...
        if (null != updateVersion) {
            if (!password.equals(member.getMemPassword())) {
                ...
                flag = memberService.updateMemberInfo(member);
                if (flag > 0) {
                    jb.put("success", true);
                } else {
                    jb.put("success", false);
        ...
        if (null != student) {
            ...
            flag = memberService.updateMemberInfo(student);
            if (flag > 0) {
                jb.put("success", true);
            ...
            flag = memberService.updateMemberInfo(adm);
            if (flag > 0) {
                jb.put("success", true);
                jb.put("success", true);
